...
%Plotting Torus
xf = (k+cos(P)).*cos(T);yf = (k+cos(P)).*sin(T);zf = sin(P);
figure
surf(xf,yf,zf)
axis equal
shading interp
...
if c == 0
   ...
   z = sin(P).*cos(T-T);x = (k+cos(P))*cos(theta1);y =(k+cos(P))*sin(theta1);
   z1 = sin(P).*cos(T-T);x1 = (k+cos(P))*cos(theta1+(pi/6));y1 =(k+cos(P))*sin(theta1+(pi/6));
   z2 = sin(P).*cos(T-T);x2 = (k+cos(P))*cos(theta1+(pi));y2 =(k+cos(P))*sin(theta1+(pi));
   hold on     
...
[t,y] = ode45(dydt , [phi1*pi/180,phi2*pi/180],theta1);
  ...
  hold on
  for i=1:a+1
   xt1 = (k+cos(t)).*cos(y+(i-1)*p);yt1 = (k+cos(t)).*sin(y+(i-1)*p);zt1 = sin(t);
...
z1 =sin(P-P); x1 = (k-1)*cos(P);y1 =(k-1)*sin(P);
hold on
...
hold on
for i=1:a+1
   xt1 = (k+cos(t)).*cos(y+(i-1)*p);yt1 = (k+cos(t)).*sin(y+(i-1)*p);zt1 = sin(((-1)^(i-1))*t);
     plot3(xt1,yt1,zt1,'r','linewidth',1)
end
hold on

z2 =sin(acosd(c-k)*pi/180)*cos(T-T);z3 =-sin(acosd(c-k)*pi/180)*cos(T-T); x2 =c*cos(P);y2 =c*sin(P);
...
elseif c == k+1
    z =0*cos(P-P); x = c*cos(P);y =c*sin(P);
      hold on
